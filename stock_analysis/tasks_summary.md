...
| sector_collect | 板块日数据采集 (手动) | 手动执行 | `daily_task.py` |
...
| daily_compress_agent | 每日复盘压缩 (未注册定时任务) | 手动执行 | `daily_compress_agent.py` |
| weekly_audit_agent | 周审计 (未注册定时任务) | 手动执行 | `weekly_audit_agent.py` |

---
*自动生成于 2026-05-23 13:24 | 修改 tasks.json 后重新运行 `python tools/generate_task_bats.py`*
